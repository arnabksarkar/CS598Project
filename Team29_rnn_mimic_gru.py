...
from keras.layers import Masking, Flatten, Embedding, Dense, LSTM, TimeDistributed, GRU
  ...
  """
    ...
    if synth_data:
        ...
        if target == 'MI':
            ...
            df['blood_thinner'] = (df['heparin'] + df['enoxaparin'] + df['fondaparinux']).apply(
...
def train(model_name="team29_mach_0", synth_data=False, target='MI',
          balancer=True, predict=False, return_model=False,
          n_percentage=1.0, time_steps=14, epochs=10, attention=False, benchmark=False, isGRU=False):
    """
  ...
  """
    ...
    checkpoint_dir = "./saved_models_gru/{0}".format(model_name)
    ...
    model.save('./saved_models_gru/{0}.h5'.format(model_name))
...
def return_loaded_model(model_name="team29_mach_0"):
    print('came here')
    import keras as K
    k.disable
    loaded_model = load_model("./saved_models_gru/{0}.h5".format(model_name))
...
if __name__ == "__main__":
    ...
    # Team29: Below Code was simplified.
    target = ['SEPSIS'] # , 'SEPSIS', 'MI', 'VANCOMYCIN'
    model_name = 'team29_mach_final_no_mask_'
    percent = [0.80, 0.60, 0.40, 0.20, 0.10,0.05]
    for t in target:
        if t == 'VANCOMYCIN':
            for p in percent:
                print(f'###### Starting training VANCOMYCIN for percent {p} ######')
                model_name_input = model_name + t + '_pad14_' + str(p * 100) + 'percent'
                train(model_name=model_name_input,epochs=14, synth_data=False, predict=True, target=t,
                      time_steps=14, n_percentage=p, isGRU=True)
                K.clear_session()
            print(f'###### Starting training VANCOMYCIN for benchmark ######')
            train(model_name='team29_mach_final_no_mask_VANCOMYCIN_pad14_bench',
                  epochs=14, synth_data=False, predict=True, target=t,
                  time_steps=14, benchmark=True, isGRU=True)
            K.clear_session()
            print(f'###### Starting training VANCOMYCIN for attention ######')
            train(model_name='team29_mach_final_no_mask_VANCOMYCIN_pad14_attention', epochs=14,
                  synth_data=False, predict=True, target=t, time_steps=14,
                  attention=True, isGRU=True)
            K.clear_session()
            print(f'###### Starting training VANCOMYCIN for 100 percent ######')
            train(model_name='team29_mach_final_no_mask_VANCOMYCIN_pad14',
                  epochs=14, synth_data=False, predict=True, target=t, time_steps=14, isGRU=True)
            K.clear_session()

        if t == 'MI':
            for p in percent:
                print(f'###### Starting training MI for percent {p} ######')
                model_name_input = model_name + t + '_pad14_' + str(p * 100) + 'percent'
                train(model_name=model_name_input,epochs=13, synth_data=False, predict=True, target=t,
                      time_steps=14, n_percentage=p, isGRU=True)
                K.clear_session()
            print(f'###### Starting training MI for benchmark ######')
            train(model_name='team29_mach_final_no_mask_MI_pad14_bench',
                  epochs=14, synth_data=False, predict=True, target=t,
                  time_steps=14, benchmark=True, isGRU=True)
            K.clear_session()
            print(f'###### Starting training MI for attention ######')
            train(model_name='team29_mach_final_no_mask_MI_pad14_attention', epochs=14,
                  synth_data=False, predict=True, target=t, time_steps=14,
                  attention=True, isGRU=True)
            K.clear_session()
            print(f'###### Starting training MI for 100 percent ######')
            train(model_name='team29_mach_final_no_mask_MI_pad14',epochs=14, synth_data=False, predict=True, target=t,
                  time_steps=14, isGRU=True)
            K.clear_session()

        if t == 'SEPSIS':
            for p in percent:
                print(f'###### Starting training SEPSIS for percent {p} ######')
                model_name_input = model_name + t + '_pad14_' + str(p * 100) + 'percent'
                train(model_name=model_name_input,epochs=14, synth_data=False, predict=True, target=t,
                      time_steps=14, n_percentage=p, isGRU=True)
                K.clear_session()
            print(f'###### Starting training SEPSIS for benchmark ######')
            train(model_name='team29_mach_final_no_mask_SEPSIS_pad14_bench',
                  epochs=14, synth_data=False, predict=True, target=t,
                  time_steps=14, benchmark=True, isGRU=True)
            K.clear_session()
            print(f'###### Starting training SEPSIS for attention ######')
            train(model_name='team29_mach_final_no_mask_SEPSIS_pad14_attention', epochs=14,
                  synth_data=False, predict=True, target=t, time_steps=14,
                  attention=True, isGRU=True)
            K.clear_session()
            print(f'###### Starting training SEPSIS for 100 percent ######')
            train(model_name='team29_mach_final_no_mask_SEPSIS_pad14',epochs=14, synth_data=False, predict=True, target=t,
                  time_steps=14, isGRU=True)
            K.clear_session()
